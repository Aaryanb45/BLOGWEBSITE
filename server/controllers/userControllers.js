...
const changeAvatar = async (req, res, next) => {
    try {
        if(!req.files.avatar){
            return next(new HttpError("Please chhose an image.",422))
        }
...
const editUser = async (req, res, next) => {
    try{
        ...
        // we wnat to update other details with/without changing the eamil (which is a unique id because we use it to login)
        ...
        if(!validateUserPassword){
            return next(new HttpError("Invalid current password",402))
        }


        //comapere new password
        if(newPassword !== confirmNewPassword)
            {
                return next(new HttpError("New passsword do not match.",422))
